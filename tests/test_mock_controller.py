...
class MockControllerTestCase(tcpip.BaseOneClientServerTestCase):
    ...
    async def test_too_many_command_parameters(self) -> None:
        async with self.create_mtdomecom_controller(), self.create_client():
            await self.write(
                command=mtdomecom.CommandName.MOVE_AZ,
    ...
    async def test_crawlAz(self) -> None:
        async with self.create_mtdomecom_controller(), self.create_client():
            ...
            target_velocity = math.radians(0.1)
            await self.write(
                command=mtdomecom.CommandName.CRAWL_AZ,
    ...
    async def test_stopAz(self) -> None:
        async with self.create_mtdomecom_controller(), self.create_client():
            ...
            await self.write(command=mtdomecom.CommandName.STOP_AZ, parameters={})
    ...
    async def test_stopEl(self) -> None:
        async with self.create_mtdomecom_controller(), self.create_client():
            ...
            await self.write(command=mtdomecom.CommandName.STOP_EL, parameters={})
            self.data = await self.read()
            assert self.data["response"] == mtdomecom.ResponseCode.OK
            assert self.data["timeout"] == pytest.approx(0.0)
    ...
    async def prepare_louvers(
        ...
        for louver_id in louver_ids:
            self.mock_ctrl.lcs.current_state[louver_id] = MotionState.MOVING.name
        self.data = await self.read()
        assert self.data["response"] == mtdomecom.ResponseCode.OK
        assert self.mock_ctrl is not None
    ...
    async def verify_louvers(
        ...
        for index, status in enumerate(lcs_status["status"]["status"]):
            if index in louver_ids:
                ...
                assert mtdomecom.InternalMotionState.STATIONARY.name == status
    ...
    async def test_stopLouvers(self) -> None:
        async with self.create_mtdomecom_controller(), self.create_client():
            ...
            await self.write(command=mtdomecom.CommandName.STOP_LOUVERS, parameters={})
            self.data = await self.read()
            assert self.data["response"] == mtdomecom.ResponseCode.OK
            assert self.data["timeout"] == mtdomecom.MockMTDomeController.LONG_DURATION
    ...
    async def test_openShutter(self) -> None:
        async with self.create_mtdomecom_controller(), self.create_client():
            ...
            await self.write(command=mtdomecom.CommandName.OPEN_SHUTTER, parameters={})
            self.data = await self.read()
            assert self.data["response"] == mtdomecom.ResponseCode.OK
            assert self.mock_ctrl is not None
    ...
    async def validate_apscs(
        ...
        await self.write(command=mtdomecom.CommandName.STATUS_APSCS, parameters={})
        self.data = await self.read()
        apscs_status = self.data[mtdomecom.LlcName.APSCS.value]
    ...
    async def test_closeShutter(self) -> None:
        async with self.create_mtdomecom_controller(), self.create_client():
            ...
            await self.write(command=mtdomecom.CommandName.CLOSE_SHUTTER, parameters={})
            self.data = await self.read()
            assert self.data["response"] == mtdomecom.ResponseCode.OK
            assert self.mock_ctrl is not None
            assert self.data["timeout"] == pytest.approx(0.0)
    ...
    async def validate_operational_mode(
        self, llc: mtdomecom.mock_llc.BaseMockStatus, command_part: str
    ) -> None:
    ...
    async def test_config(self) -> None:
        async with self.create_mtdomecom_controller(), self.create_client():
            ...
            parameters = {
                "system": mtdomecom.LlcName.AMCS.value,
                "settings": [
            ...
            parameters = {
                "system": mtdomecom.LlcName.LWSCS.value,
                "settings": [
    ...
    async def test_park(self) -> None:
        async with self.create_mtdomecom_controller(), self.create_client():
            ...
            await self.write(command=mtdomecom.CommandName.PARK, parameters={})
            self.data = await self.read()
            assert self.data["response"] == mtdomecom.ResponseCode.OK
    ...
    async def test_setTemperature(self) -> None:
        async with self.create_mtdomecom_controller(), self.create_client():
            temperature = 10.0
            await self.write(
                command=mtdomecom.CommandName.SET_TEMPERATURE,
    ...
    async def test_inflate(self) -> None:
        async with self.create_mtdomecom_controller(), self.create_client():
            ...
            self.mock_ctrl.thcs.command_time_tai = self.mock_ctrl.current_tai
            await self.write(
                command=mtdomecom.CommandName.INFLATE,
    ...
    async def test_status(self) -> None:
        async with self.create_mtdomecom_controller(), self.create_client():
            ...
            self.data = await self.read()
            cscs_status = self.data[mtdomecom.LlcName.CSCS.value]
    ...
    async def test_set_zero_az(self) -> None:
        async with self.create_mtdomecom_controller(), self.create_client():
            ...
            await self.write(command=mtdomecom.CommandName.SET_ZERO_AZ, parameters={})
            self.data = await self.read()
            assert self.data["response"] == mtdomecom.ResponseCode.INCORRECT_PARAMETERS
            assert self.data["timeout"] == -1
